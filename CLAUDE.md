...
| `paats.js` / `paats.css` | PAATS lightning-hold dispatch (SOC → truck 1/2/3, acknowledge → Parked / Couldn't-park + reason, 14-day log, `elt.paats`) + the **Ground Board**: per-ops-day (5AM→5AM) roster of aircraft on the ground (`elt.paats.ground`), dispatch outcomes auto-link back (groundId at send, tail-match at done/unable), closures derived from timestamp gaps, frozen day summaries (`elt.paats.daysum`, 400d) power the parked-percentage reports. SOC (dispatch + board) sits behind a session-cached access code; trucks are never gated. Gate changes edit the board entry AND any live dispatch (`gateHist` on the record); after acknowledge a pulsing "PAATS at the gate" step stamps `atGateAt`; the log shows the full sent→on it→at gate→parked timeline. The **Gate map** (`elt.paats.gates`) is a self-contained SVG schematic — gate areas are DRAWN BOUNDARIES (polygons: walk the corners with GPS or tap them onto the map; legacy single-spot circles still work), live GPS dot per device, and a geofence watcher on the truck screen that auto-stamps at-the-gate (`atGateBy:"gps"`) via point-in-polygon; the manual pulse stays ground truth. Storage mirror is focus-guarded (never repaints over a half-typed tail). |
| `hub.js` / `hub.css` | The Move Team Hub as a kit screen — square tile grid (home-tile family) in groups (Operations / Field logging / Lookups / App) routing through `goTab`; honors the tile-visibility toggles. |
